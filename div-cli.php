...
require "lib/div.php";
require "lib/divNodes.php";
require "lib/divWays.php";
include_once "bin/helpers.php";
...
foreach ($repos as $repo) div::addIncludePath($repo['destination']);

// Commands implementation
$commands = [];

$bin_dir = scandir(__DIR__ . "/bin");

foreach ($bin_dir as $entry) {
    if ($entry != ".." && $entry != ".")
        @include_once "bin/$entry";
}

// Starter
$sys_args = $_SERVER['argv'];
foreach ($sys_args as $k => $v)
    if (empty(trim($v))) unset($sys_args[$k]);
...
do {
    ...
    foreach ($prompt as $index => $value)
        if (trim($value) == "") unset($prompt[$index]);
